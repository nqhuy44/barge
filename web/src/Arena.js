...
export class Arena {
  ...
  createRandomSquareMap() {
    ...
    const density = this.seededRandom() * 0.2;

    console.log(
      `Generating TERRAIN map (Seed: ${this.seed}, Density: ${(
        density * 100
      ).toFixed(1)}%)`
    );
    ...
    // Ensure at least 1 obstacle if density > 0.05
    let OBSTACLE_COUNT = Math.floor((totalArea / cellArea) * density);
    if (density > 0.05 && OBSTACLE_COUNT < 1) OBSTACLE_COUNT = 1;

    console.log(
      `Map Gen: R=${this.radius}, Density=${density.toFixed(
        2
